...
import com.nju.edu.erp.service.Impl.strategy.promotion.CustomerLevelStrategy;
...
public class SaleServiceImpl implements SaleService {
    ...
    @Autowired
    public SaleServiceImpl(SaleSheetDao saleSheetDao, ProductDao productDao, CustomerDao customerDao, ProductService productService, CustomerService customerService, WarehouseService warehouseService, VoucherDao voucherDao, LevelDiscountDao levelDiscountDao, PromotionContext promotionContext) {
    ...
    public void makeSaleSheet(UserVO userVO, SaleSheetVO saleSheetVO) {
        if (saleSheetVO.getVoucherId() == null) {
            saleSheetVO.setVoucherAmount(BigDecimal.ONE);
        }
        if (saleSheetVO.getDiscount() == null) {
            saleSheetVO.setDiscount(BigDecimal.ZERO);
        }
        ...
        // 使用促销策略计算折让后总价
        promotionContext.setFinalAmount(saleSheetPO);
        saleSheetDao.saveSheet(saleSheetPO);
    }

    private BigDecimal calculateFinalAmount(SaleSheetPO saleSheetPO) {
        return saleSheetPO.getRawTotalAmount().multiply(saleSheetPO.getDiscount()).subtract(saleSheetPO.getVoucherAmount());
    }
    ...
    public List<SaleSheetVO> getSaleSheetByState(SaleSheetState state) {
        ...
        if (state == null) {
            ...
            all = saleSheetDao.findSheetByState(state);
        }
        for (SaleSheetPO po : all) {
            SaleSheetVO vo = new SaleSheetVO();
            BeanUtils.copyProperties(po, vo);
            List<SaleSheetContentPO> contents = saleSheetDao.findContentBySheetId(po.getId());
            List<SaleSheetContentVO> vos = new ArrayList<>();
            for (SaleSheetContentPO p : contents) {
    ...
    public void approval(String saleSheetId, SaleSheetState state) {
        ...
        if (state.equals(SaleSheetState.FAILURE)) {
            ...
            if (state.equals(SaleSheetState.SUCCESS)) {
                ...
                // 改变代金券状态
                Integer voucherId = saleSheetDao.findSheetById(saleSheetId).getVoucherId();
                voucherDao.use(voucherId);
